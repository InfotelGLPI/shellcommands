...
function plugin_shellcommands_install()
{
    ...
    if (!$DB->tableExists("glpi_plugin_cmd_profiles")
    ...
    } elseif ($DB->tableExists("glpi_plugin_cmd_profiles")
        ...
        $DB->runFile(PLUGIN_SHELLCOMMANDS_DIR . "/sql/update-1.2.0.sql");
        $DB->runFile(PLUGIN_SHELLCOMMANDS_DIR . "/sql/update-1.3.0.sql");
    } elseif (!$DB->tableExists("glpi_plugin_shellcommands_shellcommands")) {
        $update = true;
        $DB->runFile(PLUGIN_SHELLCOMMANDS_DIR . "/sql/update-1.3.0.sql");
    } elseif (!$DB->tableExists("glpi_plugin_shellcommands_commandgroups")) {
...
function plugin_shellcommands_getDatabaseRelations()
{
    if (Plugin::isPluginActive("shellcommands")) {
        return [
            "glpi_plugin_shellcommands_shellcommandpaths" => ["glpi_plugin_shellcommands_shellcommands" => "plugin_shellcommands_shellcommandpaths_id"],
...
function plugin_shellcommands_MassiveActions($type)
{
    if (Plugin::isPluginActive("shellcommands")) {
        if (in_array($type, PluginShellcommandsShellcommand::getTypes(true))) {
            return [
                'PluginShellcommandsShellcommand' . MassiveAction::CLASS_ACTION_SEPARATOR . "generate" => __(
                    'Command launch',
                    'shellcommands'
                ),
                'PluginShellcommandsCommandGroup' . MassiveAction::CLASS_ACTION_SEPARATOR . "generate" => __(
                    'Command group launch',
                    'shellcommands'
                )
            ];
        }
